...
test("localized home and activity are server rendered", async ({ page }) => {
  ...
  if ((page.viewportSize()?.width ?? 1200) < 992)
...
test("Payload admin is reachable while private users stay protected", async ({
  page,
  request,
}) => {
...
test("legacy reference site is no longer served", async ({ request }) => {
  const response = await request.get("/legacy/index.html", { maxRedirects: 0 });

...
export async function GET(request: Request) {
  ...
  if (!from || !to || !slug || !isLocale(from) || !isLocale(to))
    return NextResponse.json(
      { error: "Invalid locale switch request" },
      { status: 400 },
    );
  ...
  if (!target)
    return NextResponse.json(
      { error: "Target translation not published" },
      { status: 404 },
    );

  return NextResponse.redirect(
    new URL(`${localizedPath(to, "activities")}/${target.slug}`, request.url),
    307,
  );
}

...
export function LanguageSwitcher({locale}: {locale: Locale}) {
  ...
  function href(target: Locale) {
    ...
    if (page === 'activities' && segments[1]) {
      const query = new URLSearchParams({from: locale, to: target, slug: segments[1]});
      return `/api/i18n/switch?${query.toString()}`;
    }
    return localizedPath(target,page);
  }
  return <li className="nav-item dropdown language-switcher" ref={root}><button className="nav-link dropdown-toggle" type="button" aria-expanded={open} aria-label="Language" onClick={() => setOpen((value) => !value)}>{locale.toUpperCase()}</button><ul className={`dropdown-menu dropdown-menu-end${open ? ' show' : ''}`}>{locales.map((target) => <li key={target}>{target === locale ? <span className="dropdown-item active" aria-current="page">{target.toUpperCase()}</span> : <Link className="dropdown-item" href={href(target)} hrefLang={target} prefetch={false} onClick={() => setOpen(false)}>{target.toUpperCase()}</Link>}</li>)}</ul></li>;
}
